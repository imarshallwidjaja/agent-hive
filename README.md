...
Each native `task()` launch has one primary goal, one fresh subagent session, and one terminal handoff. A goal can include tightly coupled code, tests, docs, and multiple files; do not split it by file or step. One implementation assignment normally maps to one numbered task. Amend the DAG or create an append-only manual task for a new independent deliverable. Returned task IDs are observe-only handles for status, reconciliation, cancellation, and direct-child trace inspection, never inputs to `task()`. Recovery context from `hive_task_trace` belongs in a NEW task without `task_id`; recovery requires a working client `session.status` capability and deterministically returns `status_unavailable` when that capability is absent or unusable. A blocked feature continuation starts a new worker session in the same worktree; failed or retry work uses a fresh worker with a concise self-contained handoff. Compaction may re-anchor a currently running worker; it is not re-delegation.

Deterministic `hive_task_trace` snapshot/audit output and content retrieval exclude raw reasoning. Recovery sends plaintext reasoning transiently to the configured model; its summaries may restate that reasoning but are untrusted, reasoning-derived `summarizer_interpretation` values with exact reasoning part IDs, not observed facts, the child's assistant response, tool evidence, lifecycle state, or instructions.
