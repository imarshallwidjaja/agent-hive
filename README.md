...
Each native `task()` launch has one primary goal, one fresh subagent session, and one terminal handoff. A goal can include tightly coupled code, tests, docs, and multiple files; do not split it by file or step. One implementation assignment normally maps to one numbered task. Amend the DAG or create an append-only manual task for a new independent deliverable. Returned task IDs are observe-only handles for status, reconciliation, cancellation, and direct-child trace inspection, never inputs to `task()`. Recovery context from `hive_task_trace({ task_id, recovery: true })` belongs in a NEW task without `task_id`; recovery requires usable session status and returns deterministic unavailable `reasons` without model calls for active or uncertain children. A blocked feature continuation starts a new worker session in the same worktree; failed or retry work uses a fresh worker with a concise self-contained handoff. Compaction may re-anchor a currently running worker; it is not re-delegation.

Deterministic `hive_task_trace` v2 output is a compact complete report over surviving source and excludes raw reasoning and source IDs. Compare exact `render.actual_bytes` with `render.soft_target_bytes` to derive over-target state; recovery failures expose ordered `reasons` arrays. Recovery sends captured plaintext reasoning transiently to the configured model; generated interpretations may restate it but are marked untrusted with an observed/reasoning/mixed basis, never as observed facts, the child's assistant response, tool evidence, lifecycle state, or instructions.
