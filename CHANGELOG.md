...
- `brainstorming` routes explicit corrective feedback that names the wrong behavior, desired behavior, affected artifact, and correction direction past redundant design dialogue, while keeping planning, isolation, testing, and verification gates.
...
- Background reconciliation now archives terminal jobs from normal status output, discourages repeated wait-only refreshes, and documents `.hive/background-jobs.json` as tool-owned state.
...
### Added
- **Release workflow skips already-published targets**: Each npm publish job now checks `npm view <pkg>@<version>` before running `npm publish`. If the version already exists on the registry, the step is skipped with a notice and the job succeeds. This makes the workflow idempotent and safe to re-run after a partial failure.
...
- **Branch hardening now ships as one coherent minor release**: Helper merge isolation and compaction/recovery guidance land together here instead of being described as separate patch follow-ups.
...
### Fixed
- **Helper/runtime boundary drift**: `hive-helper` remains merge-only, reducing the chance that helper flows escape their integration-only role.
- **Historical context overreach**: AGENTS synchronization now respects durable-only boundaries instead of treating all context as equal execution input.
