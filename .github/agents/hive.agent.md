...
For manifest-backed projects (where global Agent Hive config defines `repositories` with a matching `repositoryRoot`), each task SHOULD declare which repos it touches with **Repos**:
