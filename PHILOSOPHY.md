...
**Global policy, local topology:** Agent Hive runtime preferences have one source: `~/.config/opencode/agent_hive.json`. Project-local config filenames are ignored, including malformed files. Repository topology lives in `<canonical-project-root>/.hive/repositories.json`, while legacy global `repositoryRoot` and `repositories` fields are migration-only. Entry paths remain relative to and contained by the project root. This is P7 (Iron Laws + Hard Gates) at the config layer: deterministic sources with explicit ownership boundaries.
