...
**Global-only config:** Agent Hive runtime configuration has one source: `~/.config/opencode/agent_hive.json`. Project-local config filenames are ignored, including malformed files. Repository manifests stay safe in the global file through a canonical `repositoryRoot` scope, while entry paths remain relative to and contained by that root. This is P7 (Iron Laws + Hard Gates) at the config layer: one deterministic source and an explicit activation boundary.
