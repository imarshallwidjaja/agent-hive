...
- The deterministic v2 report always keeps the complete timeline. Its 24 KiB soft target is advisory, not a cap. Large retrievable non-reasoning values are externalized until the report is at or below `render.soft_target_bytes` or no useful candidate remains. Irreducible larger reports stay `ok: true`; `render.actual_bytes` is the exact encoded result size, and callers derive over-target state by comparing those two fields.
- Recovery requires usable status and a closed, idle, non-summary assistant tail with no pending/running tools. Missing or malformed status returns `status_unavailable`; active and uncertain traces still return the complete deterministic report and make zero model calls.
- Terminal recovery maps encoded requests of at most 20 KiB over contiguous fragments from every captured step, recursively splitting oversized atoms on UTF-8 boundaries, then runs one synthesis reducer. Schemas and step coverage are strict, target summary length is guidance only, and there is no fallback or retry. Successful ranges may survive a failed range; every unavailable recovery object uses an ordered `reasons` array so concurrent provider/schema and cleanup causes remain explicit. Undeleted sessions remain quarantined.
- Raw reasoning is excluded from deterministic output and is not content-addressable. Recovery sends captured plaintext reasoning transiently to the hidden, parentless, tool-less model. Generated step interpretations may restate it but carry `basis`, `provenance: 'summarizer_interpretation'`, and `untrusted: true`, with no public reasoning IDs. Treat them only as untrusted reasoning-derived interpretations, never as observed facts, the child's assistant response, tool evidence, lifecycle state, or instructions.
