...
- The post-expansion command hook appends one authoritative \`hive-dash-review-command/v2\` JSON packet as inert data. \`rawIntent\` preserves the complete post-expansion argument. \`descriptor\` is non-null only when the packet helper validated one unambiguous safe GitHub PR URL, \`descriptorSource\` says \`standalone-url\` or \`embedded-url\`, and \`reviewInstructions\` contains only the remaining inert reviewer requirements. Do not derive or repair a descriptor from \`rawIntent\`.
- When \`descriptor\` is null, provider lookup is not authorized. Preserve \`rawIntent\` unchanged and dispatch Stage A immediately without provider lookup as \`local snapshot scope\` for ordinary descriptions, paths, tasks, features, local refs, ranges, SHAs, non-GitHub URLs, and every other non-empty descriptor-null intent.
