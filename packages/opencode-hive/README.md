...
Primary orchestrators can call `hive_task_trace({ task_id, recovery? })` for one directly delegated native OpenCode child. The v2 response is a compact, complete situation report over all surviving source in API order: lifecycle and source fidelity, instruction and latest outputs, aggregate reasoning counts, deduplicated tools, structured errors, patch file names, open tools, and one ordered timeline entry per normalized step. Raw reasoning and source IDs are never exposed. The 24 KiB render target is advisory; retrievable non-reasoning values are externalized first, but an irreducible larger report remains `ok: true` and complete. `render.actual_bytes` is exact, and callers derive over-target state by comparing it with `render.soft_target_bytes`. `recovery: true` is terminal-only and runs strict internal map/reduce over the same captured IR. Active or uncertain traces remain complete and return deterministic unavailable `reasons` without model calls. Successful interpretations are marked `provenance: 'summarizer_interpretation'` and `untrusted: true`; failed map ranges and synthesis failures use ordered `reasons` arrays that preserve concurrent provider/schema and cleanup causes. `hive_task_trace_content({ task_id, content_id, offset? })` reauthorizes and re-reads one allowlisted non-reasoning source field in UTF-8-safe 8 KiB chunks. Agent Hive creates no copied trace store. See `docs/HIVE-TOOLS.md` for the full contract.
